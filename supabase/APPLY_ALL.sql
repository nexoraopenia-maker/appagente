...
-- ┌─────────────────────────────────────────
-- │ supabase/migrations/20260727000001_default_timezone_lima.sql
-- └─────────────────────────────────────────
-- ============================================================================
-- 20260727000001_default_timezone_lima.sql
-- Cambia la zona horaria por defecto de America/Mexico_City a America/Lima,
-- para que coincida con DEFAULT_TIME_ZONE en lib/tz.ts.
--
-- Solo afecta a organizaciones NUEVAS: ALTER COLUMN ... SET DEFAULT no toca
-- filas existentes, así que las orgs ya creadas conservan su timezone actual.
-- ============================================================================

alter table public.organizations
  alter column timezone set default 'America/Lima';

-- Mismo default en el parámetro de la RPC de onboarding (ver migración 0003).
-- Se recrea la función completa porque Postgres no permite cambiar el default
-- de un parámetro sin redefinirla.
create or replace function public.create_organization_for_user(
  org_name text,
  org_slug text,
  full_name text default null,
  org_timezone text default 'America/Lima'
)
returns uuid
language plpgsql
security definer
